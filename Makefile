...
.PHONY: build
build: ## Builds binary of application code and stores in bin directory as dis-bundle-api
	go build -tags 'production' $(LDFLAGS) -o $(BINPATH)/dis-bundle-api
...
debug: ## Used to run code locally in debug mode
	go build -tags 'debug' $(LDFLAGS) -o $(BINPATH)/dis-bundle-api
	HUMAN_LOG=1 DEBUG=1 $(BINPATH)/dis-bundle-api
