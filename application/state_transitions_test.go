...
func createStateTransitionsForHandler(handler TransitionHandler, sourceState, targetState models.BundleState) []StateMachineTransition {
...
func TestHandleInReviewToApproved_success(t *testing.T) {
	ctx := context.Background()

	states := getMockStates()
	transitions := getMockTransitions()
	handlers := createStateTransitionsForHandler(handleInReviewToApproved, models.BundleStateInReview, models.BundleStateApproved)
	...
	Convey("When transitioning from 'IN_REVIEW' to 'APPROVED' with bundle contents APPROVED", t, func() {
		err := stateMachine.Transition(ctx, stateMachineBundleAPI, currentBundleWithStateInReview, bundleStateApproved)
...
func TestHandleInReviewToApproved_failure(t *testing.T) {
	ctx := context.Background()
	...
	stateMachine := NewStateMachine(ctx, states, transitions, store.Datastore{Backend: mockedDatastore}, handlers)
	stateMachineBundleAPI := Setup(store.Datastore{Backend: mockedDatastore}, stateMachine, datasetstests.CreateDatasetsClientMock())
	...
	Convey("When transitioning from 'IN_REVIEW' to 'APPROVED' with bundle contents not APPROVED", t, func() {
		Convey("And CheckAllBundleContentsAreApproved returns false", func() {
			...
			Convey("Then the transition should fail", func() {
				...
				So(err.Description, ShouldEqual, "not all bundle contents are approved")
			})
		})
		...
		Convey("And CheckAllBundleContentsAreApproved returns an error", func() {
			...
			Convey("Then the transition should fail with an error", func() {
				err := stateMachine.Transition(ctx, stateMachineBundleAPI, currentBundleWithStateInReview, bundleStateApproved)
				So(err, ShouldNotBeNil)
				So(err.Description, ShouldEqual, "datastore error")
			})
		})
	})
}
...
func createMockDatasetsApi(contentItems map[string][]*ContentItemVersion, getForContentItemsError *string, updateStateForContentItemError *string) datasetsmock.DatasetsVersionsClientMock {
	...
	return datasetsmock.DatasetsVersionsClientMock{
		GetForContentItemFunc: func(ctx context.Context, contentItem models.ContentItem) (*datasetsmodels.Version, error) {
		...
		UpdateStateForContentItemFunc: func(ctx context.Context, contentItem models.ContentItem, targetState models.BundleState) error {
...
func TestHandleApprovedToPublished_success(t *testing.T) {
	...
	handlers := createStateTransitionsForHandler(handleApprovedToPublished, models.BundleStateApproved, models.BundleStatePublished)
	...
	Convey("When transitioning from 'APPROVED' to 'PUBLISHED' with all valid matching states", t, func() {
		...
		err := stateMachine.Transition(ctx, stateMachineBundleAPI, testBundle, bundleStatePublished)
