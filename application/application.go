...
func Setup(datastore store.Datastore, stateMachine *StateMachine, datasetsClient datasets.DatasetsClient) *StateMachineBundleAPI {
...
func (s *StateMachineBundleAPI) UpdateBundleState(ctx context.Context, bundleID string, state models.BundleState, suppliedEtag string) (*models.Bundle, *models.Error) {
	bundle, err := s.getBundleAndValidateETag(ctx, bundleID, suppliedEtag)
	if err != nil {
		return nil, err
	}

	transitionError := s.StateMachine.Transition(ctx, s, bundle, state)
