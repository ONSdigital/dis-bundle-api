...
func TestGetBundleEvents_Success(t *testing.T) {
	Convey("Given a successful request with no query parameters", t, func() {
		...
		stateMachineBundleAPI := application.Setup(store.Datastore{Backend: mockDatastore}, stateMachine, mockDatasetAPIClient, mockPermissionsAPIClient, mockSlackClient, "", "", 59*time.Second)
