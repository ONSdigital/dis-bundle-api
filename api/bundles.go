...
func (api *BundleAPI) getBundles(w http.ResponseWriter, r *http.Request, limit, offset int) (successResult *models.PaginationSuccessResult[models.Bundle], errorResult *models.ErrorResult[models.Error]) {
	...
	filters, err := filters.CreateBundlefilters(r)
	if err != nil {
		log.Error(ctx, err.Error(), errs.ErrInvalidQueryParameter)
		code := models.CodeInternalServerError
		invalidRequestError := &models.Error{Code: &code, Description: errs.ErrorDescriptionMalformedRequest}
	...
	if totalCount == 0 && filters.PublishDate != nil {
		...
		notFoundError := &models.Error{Code: &code, Description: errs.ErrorDescription404}
